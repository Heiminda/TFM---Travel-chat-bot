...
class Batcher(object):
  ...
  def __init__(self, data_path, vocab, config,
    ...
    self._input_queue = Queue.Queue(QUEUE_NUM_BATCH * self._config['batch_size'])
  ...
  def NextBatch(self):
    ...
    """
    enc_batch = np.zeros(
        (self._config['batch_size'], self._config['input_seq_length']), dtype=np.int32)
    enc_input_lens = np.zeros(
        (self._config['batch_size']), dtype=np.int32)
    dec_batch = np.zeros(
        (self._config['batch_size'], self._config['output_seq_length']), dtype=np.int32)
    dec_output_lens = np.zeros(
        (self._config['batch_size']), dtype=np.int32)
    target_batch = np.zeros(
        (self._config['batch_size'], self._config['output_seq_length']), dtype=np.int32)
    loss_weights = np.zeros(
        (self._config['batch_size'], self._config['output_seq_length']), dtype=np.float32)
    origin_articles = ['None'] * self._config['batch_size']
    origin_abstracts = ['None'] * self._config['batch_size']

    buckets = self._bucket_input_queue.get()
    for i in xrange(self._config['batch_size']):
  ...
  def _FillInputQueue(self):
    ...
    while True:
      ...
      if (len(enc_inputs) < self._config['min_input_len'] or
          len(dec_inputs) < self._config['min_input_len']):
      ...
      if not self._truncate_input:
        if (len(enc_inputs) > self._config['input_seq_length'] or
            len(dec_inputs) > self._config['output_seq_length']):
          tf.logging.warning('Drop an example - too long.\nenc:%d\ndec:%d',
                             len(enc_inputs), len(dec_inputs))
          continue
      # If we are truncating input, do so if necessary
      else:
        if len(enc_inputs) > self._config['input_seq_length']:
          enc_inputs = enc_inputs[:self._config['input_seq_length']]
        if len(dec_inputs) > self._config['output_seq_length']:
          dec_inputs = dec_inputs[:self._config['output_seq_length']]
      ...
      # Pad if necessary
      while len(enc_inputs) < self._config['input_seq_length']:
        enc_inputs.append(pad_id)
      while len(dec_inputs) < self._config['output_seq_length']:
        dec_inputs.append(end_id)
      while len(targets) < self._config['output_seq_length']:
        targets.append(end_id)
  ...
  def _FillBucketInputQueue(self):
    ...
    while True:
      inputs = []
      for _ in xrange(self._config['batch_size'] * BUCKET_CACHE_BATCH):
        inputs.append(self._input_queue.get())
      if self._bucketing:
        inputs = sorted(inputs, key=lambda inp: inp.enc_len)

      batches = []
      for i in xrange(0, len(inputs), self._config['batch_size']):
